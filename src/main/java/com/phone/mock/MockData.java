...
import com.phone.util.DateUtils;
...
public class MockData {
    ...
    static {
         ...
         * 模拟的是注册用户浏览了电商品台上哪些型号的电子产品
         */
        productNames = new String[]{"Xiaomi/小米 小米手机6", "Apple/苹果 iPhone X", "Meizu/魅族 魅蓝 S6", "honor/荣耀 荣耀V10", "OPPO R11", "vivo X20", "Huawei/华为 Mate 10 6G+", "Samsung/三星 Galaxy S9+", "Xiaomi/小米 小米手环2", "Microsoft/微软  Xbox one 手柄"};
    }
     ...
     * @param sqlContext
     */
    public static void mock(JavaSparkContext sc, SQLContext sqlContext) throws Exception {
...
//        DataFrame dataset = sqlContext.createDataFrame(rowsRDD, schema);
//        dataset.registerTempTable("user_info");
    }
...
//        DataFrame dataset = sqlContext.createDataFrame(rowsRDD, schema);
//        dataset.registerTempTable("user_visit_action");
    }
    ...
    private static void randomAccessPage(List<Row> rows, String date, BufferedWriter bw, long userId, String sessionId) throws IOException {
        ...
        for (int k = 0; k <= random.nextInt(100); k++) {
            ...
            String searchKeyword = null;//搜索关键字
            Long clickProductId = null;//点击产品的id
            String orderCategoryIds = null;//订单（order）类型（category）的id
            String orderProductIds = null;//订单产品的id
            String payCategoryIds = null;//支付类型的id
            String payProductIds = null;//支付产品的id
            Long clickCategoryId = null;//点击类型的id
